...
try {
  ...
  router.use(
    "/public/datatables",
    express.static(
      path.join(__dirname, "node_modules/datatables.net-bs4/css"),
      { maxAge: oneDay }
    )
  );
  router.use(
  ...
  router.use(
    "/public/moment/locale",
    express.static(path.join(__dirname, "node_modules/moment/locale"), {
      maxAge: oneDay,
    })
  );
  router.use(
  ...
  router.use(
    "/public/datepicker",
    express.static(
      path.join(__dirname, "node_modules/bootstrap-datepicker/dist/css"),
      { maxAge: oneDay }
    )
  );
  router.use(
  ...
  router.use(
    "/public/multiselect",
    express.static(
      path.join(__dirname, "node_modules/bootstrap-multiselect/dist/js"),
      { maxAge: oneDay }
    )
  );
  router.use(
    "/public/multiselect",
    express.static(
      path.join(__dirname, "node_modules/bootstrap-multiselect/dist/css"),
      { maxAge: oneDay }
    )
  );
  router.use(
  ...
  router.use(
    "/public/datatables",
    express.static(
      path.join(__dirname, "node_modules/datatables.net-responsive-bs4/js"),
      { maxAge: oneDay }
    )
  );
  router.use(
    "/public/datatables",
    express.static(
      path.join(__dirname, "node_modules/datatables.net-responsive-bs4/css"),
      { maxAge: oneDay }
    )
  );
  router.use(
  ...
  app.use(function (req, res, next) {
    ...
    res.locals.siteurl =
      "https://" + req.hostname + utils.generateUrl(req, "/");
    res.locals.siteurl = res.locals.siteurl.substring(
      0,
      res.locals.siteurl.length - 1
    );
    res.locals.replaceUmlaute = replaceUmlaute;
    res.locals.format = require("./utils/format");
    res.locals.sprintf = require("sprintf-js").sprintf;
  ...
  if (settings.config.get("site.https") === "true") {
    var privateKey = fs.readFileSync(
      settings.config.get("site.sslkey"),
      "utf8"
    );
    var certificate = fs.readFileSync(
      settings.config.get("site.sslcert"),
      "utf8"
    );
    var credentials = { key: privateKey, cert: certificate };
    console.log(
      "starting https server on: " +
        settings.config.get("site.porthttps").toString()
    );
  ...
  if (settings.config.get("site.http") === "true") {
    console.log(
      "starting http server on: " +
        settings.config.get("site.porthttp").toString()
    );
